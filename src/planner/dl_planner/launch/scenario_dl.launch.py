#!/usr/bin/env python3
"""
Scenario launch file for DL Planner with Ego-Swarm integration
...
def generate_launch_description():
    ...
    # DL planner uses learned collision network
    drone_configs = [
    ...
    # Drone nodes - Each runs DL planner + simulator
    drone_nodes = []

    for config in drone_configs:
        drone_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('dl_planner'), 'launch', 'run_in_sim_dl.launch.py')),
            launch_arguments={
    ...
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
        get_package_share_directory('dl_planner')))))
