#!/usr/bin/env python3
"""
Scenario launch file for ORCA Planner with Ego-Swarm integration
...
def generate_launch_description():
    ...
    # ORCA is reactive, so they start and fly to goals in real-time
    drone_configs = [
    ...
    # Drone nodes - Each runs ORCA + simulator
    drone_nodes = []

    # Get launch file path
    try:
        orca_launch_dir = get_package_share_directory('orca_planner')
    except:
        # Fallback: use ego_planner to find the path
        ego_share = get_package_share_directory('ego_planner')
        orca_launch_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(ego_share))),
            'orca_planner', 'share', 'orca_planner')

    for config in drone_configs:
        # Note: ORCA is a reactive planner, each drone runs independently
        # They avoid each other in real-time using ORCA algorithm
        drone_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(orca_launch_dir, 'launch', 'run_in_sim_orca.launch.py')),
            launch_arguments={
    ...
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(
        get_package_share_directory('orca_planner')))))
