#!/usr/bin/env python3
"""
Calculate fair Hungarian assignment for 36 drones
From FORMATION_36_A (5x5 Grid) to FORMATION_36_B (Triangle)
...
from formations_large import FORMATION_36_A, FORMATION_36_B


def main():
    print("=" * 80)
    print("Fair Hungarian Assignment for 36 Drones")
    print("From FORMATION_36_A (5x5 Grid) to FORMATION_36_B (Triangle)")
    print("=" * 80)
    ...
    assignment = allocator.allocate_fair(FORMATION_36_A, FORMATION_36_B)

    # Prepare output
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = f"assignment_36_drones_{timestamp}.txt"
    output_path = os.path.join(os.path.dirname(__file__), output_file)

    with open(output_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("Fair Hungarian Assignment for 36 Drones\n")
        ...
        f.write("=" * 80 + "\n")
        f.write("Drone Configurations for scenario_swarm_36.launch.py:\n")
        f.write("=" * 80 + "\n\n")

        f.write("drone_configs = [\n")

        for drone_id in range(36):
            init_pos = FORMATION_36_A[drone_id]
            target_idx = assignment[drone_id]
            target_pos = FORMATION_36_B[target_idx]
